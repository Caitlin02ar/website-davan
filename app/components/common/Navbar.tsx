...
export default function Navbar() {
  ...
  const handleNav = (href: string) => (e: React.MouseEvent) => {
    ...
    // kalau target sama dengan pathname sekarang tapi ada hash nyangkut, bersihkan
  ...
  return (
    <nav
      className={`
        ...
        ${
          scrolled
            ? `bg-gradient-to-b from-black/100 via-black/50 to-transparent`
    ...
    >
      <div
        className={`
          ...
          ${scrolled ? "h-16 md:h-20 md:px-10" : "h-20 md:h-24 md:px-10"}
        `}
      >
        <Link
          href="/"
        ...
        >
          <Image
            ...
            className={`
              transition-all duration-500
              ${scrolled ? "w-[120px] md:w-[150px]" : "w-[140px] md:w-[180px]"}
            `}
          />
        </Link>
            ...
            return (
              <li key={item.href}>
                <Link
                  ...
                  className={`
                    group relative pb-1 text-sm transition-colors duration-300
      ...
      >
        <div
          className="
            flex flex-col gap-6 border-t border-white/10
            bg-black/80 px-6 py-8
          "
        >
          {NavItems.map((item) => {
            const isActive = pathname === item.href;
            ...
            return (
              ...
              >
                ...
                <span
                  className={`
                    absolute bottom-[-4px] left-0 h-[1.5px] bg-primary
                    transition-all duration-300
