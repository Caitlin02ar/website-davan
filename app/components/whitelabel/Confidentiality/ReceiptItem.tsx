...
export default function ReceiptItem({
    ...
    const deliveredBy = receipt.receiptItems.find(
        (item) => item.label === "Delivered by"
    );

    const client = receipt.receiptItems.find(
        (item) => item.label === "Client"
    );

    const designAndBuild = receipt.receiptItems.find(
        (item) => item.label === "Design and build"
    );
                    ...
                    <div className="relative z-10 px-6 pb-8 pt-7 md:px-8 md:pb-9 md:pt-8">
                        ...
                        {designAndBuild && (
                            <div className="mt-5 flex items-center justify-between">
                                <span className="text-xs md:text-sm">
                                    {designAndBuild.label}
                                </span>

                                <span className="font-body text-sm font-bold text-primary md:text-base">
                                    {designAndBuild.value}
                                </span>
                            </div>
                        )}
